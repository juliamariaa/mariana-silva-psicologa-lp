...
function TestimonialCards() {
  ...
  }, [isSwiping, startX, navigate, totalCards]);
      ...
      >
        ...
        >
          {testimonialData.map((item, index) => {
            const isActive =
              index >= cardAtivo && index < cardAtivo + visibleCards;

            return (
              <div
                key={index}
                className={`shrink-0 flex flex-col justify-center text-center gap-4 bg-primary rounded px-6 py-8 md:py-14 transition-all duration-500 ${isActive ? '' : 'opacity-60'}`}
                style={{
              ...
              >
                <div className="flex flex-col items-center">
                  <img
                    ...
                    src={item.urlImage}
                    alt={`Foto de perfil de ${item.fullName}`}
                />
